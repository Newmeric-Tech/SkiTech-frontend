...
export default function StaffDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    rankingAPI.getEmployeeRanking(id, tenantId)
      .then(setRanking)
      .catch((err: any) => setError(err?.response?.data?.detail ?? "Failed to load employee ranking"))
  ...
  if (error || !ranking) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4 text-center">
        ...
        <h2 className="text-xl font-bold text-slate-950">{error ?? "No ranking found"}</h2>
        <p className="text-slate-500 text-sm">This employee may not have been ranked yet.</p>
        <button onClick={() => router.back()} className="mt-2 px-4 py-2.5 text-sm font-medium text-white bg-slate-950 rounded-xl">
          Go Back
        </button>
      </div>
    );
  ...
  const attendanceScore = ranking.scores_breakdown.find(s => s.criterion_name === "Attendance & Functionality")?.score ?? 0;
  const taskScore = ranking.scores_breakdown.find(s => s.criterion_name === "Task Completion")?.score ?? 0;
  const statusLabel = PERF_STATUS_LABEL[ranking.performance_status] ?? "Consistent";

  const whyRankText = ranking.rank === 1
  ...
  return (
    <div className="space-y-6">
      ...
      <EmployeeProfileCard
        ...
        joinDate={new Date(ranking.period_start).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <ScoreCard icon={Calendar} label="Attendance" value={`${Math.round(attendanceScore)}%`} subtext="Attendance score" color="#3B82F6" index={0} />
        <ScoreCard icon={Target} label="Task Completion" value={`${Math.round(taskScore)}%`} subtext="Tasks completed on time" color="#10B981" index={1} />
        <ScoreCard icon={CheckCircle} label="Final Score" value={`${Math.round(ranking.overall_score * 10) / 10}`} subtext={`Rank #${ranking.rank} of ${ranking.total_active_employees}`} color="#6366F1" index={2} />
        <ScoreCard
          icon={Flame}
          label="Score Change"
          value={ranking.score_change != null ? `${ranking.score_change > 0 ? "+" : ""}${ranking.score_change.toFixed(1)}` : "—"}
        ...
        <div className="bg-white rounded-2xl border border-slate-200/60 shadow-sm p-6">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-medium">{new Date(ranking.period_start).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-slate-500">End</span>
              <span className="font-medium">{new Date(ranking.period_end).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
            </div>
            <div className="flex justify-between">
