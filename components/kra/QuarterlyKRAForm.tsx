...
import {
  ...
  DollarSign, Users, Calendar, ClipboardCheck, FileText, TrendingUp,
} from "lucide-react";

interface QuarterlyKraForm {
  quarter: string;
  year: string;
...
const initialForm: QuarterlyKraForm = {
  quarter: "",
  year: "",
  revenueAmount: "",
  guestCount: "",
...
const steps = [
  ...
  { id: 3, label: "Performance", icon: TrendingUp },
...
export default function QuarterlyKRAForm({ backHref }: { backHref: string }) {
  const router = useRouter();
  const [step, setStep] = useState(1);
  const [direction, setDirection] = useState(1);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [error, setError] = useState<string | null>(null);
  const [form, setForm] = useState<QuarterlyKraForm>(initialForm);
  const [errors, setErrors] = useState<Partial<Record<keyof QuarterlyKraForm, string>>>({});
  ...
  const validateStep = (s: number): boolean => {
    ...
    if (s === 1) {
      if (!form.quarter) newErrors.quarter = "Please select a quarter";
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      formData.append("submission_type", "quarterly");
      formData.append("period_quarter", form.quarter);
      ...
      toast.success("Quarterly KRA submitted successfully!", {
        description: `${quarters.find(q => q.value === form.quarter)?.label} ${form.year}`,
        duration: 4500,
      });
  ...
  const quarterLabel = quarters.find(q => q.value === form.quarter)?.label;
  const quarterMonths = quarters.find(q => q.value === form.quarter)?.months;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-950 tracking-tight">Quarterly KRA Form</h1>
          <p className="text-slate-500 text-sm">Submit your quarterly key result areas</p>
        </div>
      </div>
            ...
            <motion.div key={step} custom={direction} variants={variants} initial="enter" animate="center" exit="exit"
              ...
              {step === 1 && (
                <>
                  <div className="bg-indigo-50 border border-indigo-200/60 rounded-xl p-4 flex items-start gap-3">
                    ...
                    <div>
                      <p className="text-sm font-semibold text-indigo-900">Quarterly Reporting Period</p>
                      <p className="text-xs text-indigo-600 mt-0.5">Select the quarter and year for this KRA submission.</p>
                    </div>
                  </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        <select value={form.quarter} onChange={(e) => setForm((f) => ({ ...f, quarter: e.target.value }))}
                          className={`w-full px-4 py-3 bg-slate-50 border rounded-xl text-sm text-slate-900 appearance-none focus:outline-none focus:ring-2 transition-all ${errors.quarter ? "border-red-300 focus:ring-red-100" : "border-slate-200/60 focus:ring-slate-200"}`}>
                  ...
                  {form.quarter && (
                    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
                      className="flex items-center gap-3 p-4 rounded-xl border border-indigo-200/60 bg-indigo-50/50">
                      <Calendar className="w-5 h-5 text-indigo-600" />
                      <div>
                        <p className="text-sm font-semibold text-indigo-900">{quarterLabel} Selected</p>
                        <p className="text-xs text-indigo-600">{quarterMonths}</p>
                      </div>
                    </motion.div>
                  )}
                </>
              ...
              {step === 2 && (
                <div className="space-y-5">
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-1.5">Financial Report File (Optional)</label>
                    <p className="text-xs text-slate-500 mb-2">Upload the quarterly financial report (PDF, Excel, or image)</p>
                    {form.revenueFilePreview ? (
                      ...
                      <div onClick={() => fileInputRef.current?.click()}
                        ...
                        <p className="text-xs text-slate-400 group-hover:text-indigo-500 transition-colors">Click to upload file</p>
              ...
              {step === 3 && (
                <div className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label className="block text-sm font-semibold text-slate-700 mb-1.5">Avg. Occupancy Rate (%)</label>
                  ...
                  <div>
                    ...
                    <textarea value={form.notes} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))}
                      placeholder="Any additional notes or observations for this quarter..." rows={4}
                ...
                <div className="space-y-4">
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/60">
                    ...
                    <div className="text-sm flex justify-between">
                      ...
                      <span className="text-slate-900 font-medium">{quarterLabel} {form.year}</span>
                    </div>
                  </div>
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/60">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5"><DollarSign className="w-3.5 h-3.5" /> Financials</p>
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 border border-slate-200/60">
                    <p className="text-xs font-semibold text-slate-500 uppercase tracking-wider mb-3 flex items-center gap-1.5"><TrendingUp className="w-3.5 h-3.5" /> Performance</p>
                    <div className="text-sm flex justify-between mb-2">
          ...
          {step < 4 ? (
            ...
            <button onClick={handleSubmit} disabled={isSubmitting}
              ...
              {isSubmitting ? (
                ...
                <><CheckCircle2 className="w-4 h-4" /> Submit Quarterly KRA</>
              )}
            </button>
          )}
        </div>
